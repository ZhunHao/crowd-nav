...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.static_obstacles = [] # save the static obstacles
        ...
        self.robot_initx = self.config.getfloat('robot', 'init_x')
        self.robot_inity = self.config.getfloat('robot', 'init_y')
    ...
    def generate_static_human(self, static_post):
        ...
        human.set(static_post[0], static_post[1], static_post[0], static_post[1], 0, 0, 0)
        return human
    ...
    def generate_random_human_position(self, human_num, rule):
        ...
        if rule == 'square_crossing':
            ...
            for i in range(human_num):
                self.humans.append(self.generate_static_human(static_post[(i)]))
        elif rule == 'mixed':
    ...
    def generate_circle_crossing_human_new(self, start_point, goal_point):
        human = Human(self.config, 'humans')
        if self.randomize_attributes:
            human.sample_random_attributes()

        # Rectangle bounds from start and goal points
        min_x = min(start_point[0], goal_point[0])
        max_x = max(start_point[0], goal_point[0])
        min_y = min(start_point[1], goal_point[1])
        max_y = max(start_point[1], goal_point[1])
        ...
        while True:
            ...
            px_noise = (np.random.random() - 0.5) * human.v_pref * 1
            py_noise = (np.random.random() - 0.5) * human.v_pref * 1
            px = center_x + radius_x * np.cos(angle) + px_noise
            py = center_y + radius_y * np.sin(angle) + py_noise
            
            # Opposite side for crossing
            gx = center_x - radius_x * np.cos(angle)
            gy = center_y - radius_y * np.sin(angle)
    ...
    def light_reset(self, phase='test', test_case=None):
        """
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            self.robot.set(self.robot_initx, self.robot_inity, self.robot_goalx, self.robot_goaly, 0, 0, np.pi / 2)
    ...
    def render(self, mode='human', goal_list=None, output_file=None):
        ...
        if mode == 'human':
            ...
            for obs in self.static_obstacles:
                if obs['type'] == 'circle':
                    ax.add_artist(plt.Circle((obs['cx'], obs['cy']), obs['r'], color='gray', alpha=0.5))
                elif obs['type'] == 'rect':
            ...
            ax.add_artist(robot)
            # ax.add_artist(goal)
            # plt.legend([robot, goal], ['Ship', 'Goal'], fontsize=16)

            if goal_list is None:
                goal = mlines.Line2D([self.robot.gx], [self.robot.gy], color=goal_color, marker='*', linestyle='None', markersize=15, label='Goal')
                ax.add_artist(goal)
                plt.legend([robot, goal], ['Ship', 'Goal'], fontsize=16, loc='upper left')
            else:
                goal_marker_list, goal_name_list = [], []
                for goal_idx in range(len(goal_list)):
                    goal_colors = plt.get_cmap('tab10').colors
                    one_goal = mlines.Line2D([goal_list[goal_idx][0]], [goal_list[goal_idx][1]],
                                             color=goal_colors[goal_idx % len(goal_colors)], marker='*',
                                             linestyle='None', markersize=15, label='Goal')
                    goal_marker_list.append(one_goal)
                    goal_name_list.append('Goal-'+str(goal_idx+1))
                    ax.add_artist(one_goal)
                    plt.legend([robot]+goal_marker_list, ['Ship']+goal_name_list, fontsize=12, loc='upper left')
            ...
            # add time annotation
            time = plt.text(-5, 13, 'Time: {}'.format(0), fontsize=16)
            ax.add_artist(time)

            # compute attention scores
            # if self.attention_weights is not None:
            #     attention_scores = [
            #         plt.text(-5.5, 5 - 0.5 * i, 'Ship {}: {:.2f}'.format(i + 1, self.attention_weights[0][i]),
            ...
            def update(frame_num):
                ...
                for i, human in enumerate(humans):
                    ...
                    if self.attention_weights is not None:
                        ...
                        # attention_scores[i].set_text('Ship {}: {:.2f}'.format(i, self.attention_weights[frame_num][i]))
